    ...
    <div className="mt-[10px] gap-[10px]">
      <div className="flex flex-row gap-5 border-b-2 content-center">
        ...
        {director.map((person, index) => {
          ...
          return (
            <p key={index} className="text-sm sm:text-xl left-[20px]">
              {person.name}{" "}
              {/* {person.job.toLowerCase().includes(  "Screenplay") && `${person.name}`} */}
            </p>
          );
        })}
      </div>
      ...
      <div className="flex flex-row mt-5 gap-5 border-b-2 content-center">
        ...
        {crew.map((person, index) => (
          <p className="text-sm sm:text-xl left-[20px]" key={index}>
            {person.name}
          </p>
        ))}
      </div>
      <div className="flex flex-row mt-5 gap-5 border-b-2 content-center">
        ...
        {cast.map((person) => (
          <p key={person.id} className="text-sm sm:text-xl left-[20px]">
            {person.name}
          </p>
        ))}
      </div>
